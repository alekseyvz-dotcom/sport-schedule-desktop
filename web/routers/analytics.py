...
def _build_result(agg):
    ...
    for (oid, oname), venues in sorted(by_org.items(), key=lambda x: x[0][1]):
        ...
        org_entry = {
            ...
            "is_total": True,
            "pct": org_pct, "pd_h": _hours(org_pd), "gz_h": _hours(org_gz),
            "busy_h": _hours(org_busy), "cap_h": _hours(org_cap),
        ...
        for v in venues:
            ...
            result.append({
                ...
                "pd_h": _hours(v["pd_sec"]), "gz_h": _hours(v["gz_sec"]),
                "busy_h": _hours(busy), "cap_h": _hours(v["capacity_sec"]),
