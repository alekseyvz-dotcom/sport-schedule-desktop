...
class DayPartsSettings:
    ...
    def from_dict(cls, data: dict | None) -> "DayPartsSettings":
        data = data or {}
        return cls(
            morning_start=str(data.get("morning_start", "08:00")),
            ...
            evening_end=str(data.get("evening_end", "22:00")),
        )
    ...
    def day_title(self) -> str:
        return f"День {self.day_start}–{self.day_end}"

    def evening_title(self) -> str:
        return f"Вечер {self.evening_start}–{self.evening_end}"
